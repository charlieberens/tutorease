...
class AddStudent extends Component {
    ...
    inviteStudent = () => {
    	axios.post(`/api/students/invite/${this.state.selected_student._id}`).then(res => {
    ...
    render() {
        return (
			<div className={`add-student-cont ${!this.state.search_value?.length && 'add-student-no-search'}`}>
				<input className="add-student-searchbar" type="text" value={this.state.search_value} onChange={this.onChange} />
				<div className="add-student-results-cont">
					{this.state.students?.map((student, index) =>
                        <div className="add-student-student" onClick={() => this.setState({selected_student: student})}>
                        	<div className="add-student-student-left">
                        		<img className="add-student-student-icon" src={student.profileIcon} />
                        	</div>
                        	<div className="add-student-student-right">
								<h3 className="add-student-displayname">{student.displayName}</h3>
	                        	<div className="add-student-student-bottom">
	                        		<p className="add-student-student-username">@{student.username}</p>
	                        	</div>
                        	</div>
                        </div>
					)}
