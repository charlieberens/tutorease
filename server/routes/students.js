    ...
    try {
        ...
        if (query_string[0] === "@") {
            ...
            User.find(
                {
                    username: {
                        $regex: new RegExp("^" + query_string + ".*$", "ig"),
                    },
                },
                {
                    username: 1,
                ...
                (err, users) => {
                    const students = users.filter(
                        (user) =>
                            user.student &&
                            user._id != user_id &&
                            !user.studentDeets.tutorRequests.includes(
                                user_id
                            ) &&
                            !user.studentDeets.tutors.includes(user_id)
                    );
                    res.json({ students });
                    return true;
                }
        ...
        User.findById(user_id, (err, tutor_user) => {
            if (err) {
                ...
                if (tutor_user.tutor) {
                    try {
                        User.find(
                            { _id: { $in: tutor_user.tutorDeets.students } },
                            {
                                displayName: 1,
                                username: 1,
                                profileIcon: 1,
        ...
        User.findById(user_id, (err, student_user) => {
            if (err) {
                ...
                if (student_user.student) {
                    try {
                        User.find(
                            { _id: { $in: student_user.studentDeets.tutors } },
                            {
                                displayName: 1,
                                username: 1,
                                profileIcon: 1,
    ...
    try {
        ...
        User.findById(user_id, (err, tutor_user) => {
            if (err) {
                ...
                let studentArr = [];
                User.find(
                    { _id: { $in: tutor_user.tutorDeets.studentRequests } },
                    { displayName: 1, username: 1, profileIcon: 1, _id: 1 },
                    (err, students) => {
        ...
        User.findById(user_id, async (err, student_user) => {
            ...
            for await (let tutor_id of Object.keys(tutor_set_obj)) {
                ...
                for (let i = 0; i < tutor_user.tutorDeets.sets.length; i++) {
                    if (
                        tutor_set_obj[tutor_id].includes(
                            tutor_user.tutorDeets.sets[i]._id.toString()
                        )
                    ) {
                        const tutor_set = tutor_user.tutorDeets.sets[i];
                        const student_set = sets.find(
                            (qset) =>
                                qset.setId.toString() ===
                                tutor_set._id.toString()
                        );
                        embiggened_sets.push({
                            title: tutor_set.title,
                            setLength: tutor_set.questions.length,
                            tutorUsername: tutor_user.username,
                            tutorDisplayName: tutor_user.displayName,
                            tutorProfileIcon: tutor_user.profileIcon,
                            setId: tutor_set._id,
                            numAnswered: student_set.questions.filter(
                                (question) => question.responses.length
                            ).length,
                            numCorrect:
                                tutor_set.questions.length ===
                                student_set.questions.filter(
                                    (question) => question.responses.length
                                ).length
                                    ? student_set.questions.filter(
                                          (question) =>
...
router.get("/set/:set_id", (req, res) => {
    try {
        ...
        User.findById(user_id, (err, student_user) => {
            if (student_user.student) {
                const student_set = student_user.studentDeets.sets.find(
                    (set) => set.setId.toString() === set_id.toString()
                );
                User.findById(student_set.tutorId, (err, tutor_user) => {
                    const tutor_set = tutor_user.tutorDeets.sets.find(
                        (set) => set._id.toString() === set_id.toString()
                    );
                    console.log({
                        student_set: student_set.questions[0].responses,
                    });
                    ...
                    res.json({
                        ...
                        id: tutor_set._id,
                        questions: tutor_set.questions.map(
                            (question, index) => ({
                                body: question.body,
                                mcq: question.mcq,
                                answers: common.shuffle(question.answers),
                                responses:
                                    student_set.questions[index].responses,
                            })
                        ),
                        title: tutor_set.title,
...
router.get("/check/:set_id/:question_index", (req, res) => {
    try {
        ...
        User.findById(user_id, (err, student_user) => {
            if (
                !student_user.studentDeets.sets.find(
                    (set) => set.setId.toString() == set_id.toString()
                ).completeDate
            ) {
                student_user.studentDeets.sets
                    .find((set) => set.setId.toString() === set_id.toString())
                    .questions[question_index].responses.push(answer);
                User.findById(
                    student_user.studentDeets.sets.find(
                        (set) => set.setId.toString() === set_id.toString()
                    ).tutorId,
                    (err, tutor_user) => {
                        const correct_answer = tutor_user.tutorDeets.sets.find(
                            (set) => set._id.toString() === set_id.toString()
                        ).questions[question_index].answers[0];
                        student_user.save();
                        if (
                            ...
                            student_user.studentDeets.sets.find(
                                (set) =>
                                    set.setId.toString() == set_id.toString()
                            ).completeDate = new Date();
                        }
                        res.json({
                            success: answer === correct_answer,
                            responses: student_user.studentDeets.sets.find(
                                (set) =>
                                    set.setId.toString() === set_id.toString()
...
// Post
router.post("/invite/:student_id", async (req, res) => {
    try {
        ...
        if (
            !student_user.studentDeets.tutorRequests.includes(tutor_id) &&
            !student_user.studentDeets.tutors.includes(tutor_id) &&
            !student_user.studentDeets.blockedTutors.includes(tutor_id)
        ) {
...
router.post("/invite/accept/:tutor_id", async (req, res) => {
    try {
        const user_id = req.session.passport.user;
        const tutor_id = req.params.tutor_id;
        const student_user = await User.findById(user_id);
        const tutorRequestsIndex =
            student_user.studentDeets.tutorRequests.indexOf(tutor_id);
        if (tutorRequestsIndex > -1) {
            student_user.studentDeets.tutors.push(tutor_id);
            student_user.studentDeets.tutorRequests.splice(
                tutorRequestsIndex,
                1
            );
            const tutor_user = await User.findById(tutor_id);
            const studentRequestsIndex =
                tutor_user.tutorDeets.studentRequests.indexOf(user_id);
            if (studentRequestsIndex > -1) {
                tutor_user.tutorDeets.students.push(user_id);
                tutor_user.tutorDeets.studentRequests.splice(
                    studentRequestsIndex,
                    1
                );
...
router.post("/assign/:student_ids/:set_id", async (req, res) => {
    try {
        const user_id = req.session.passport.user;
        ...
        const set = {
            tutorId: user_id,
            setId: set_id,
            setLength: tutor_user.tutorDeets.sets.find(
                (qset) => qset._id == set_id
            ).questions.length,
            numAnswered: 0,
            questions: Array(
                tutor_user.tutorDeets.sets.find((qset) => qset._id == set_id)
                    .questions.length
            ).fill({ responses: [] }),
        };
        if (!set.setLength) {
            ...
            try {
                for (student_id of student_ids) {
                    if (tutor_user.tutorDeets.students.includes(student_id)) {
                        User.findById(student_id, (err, student_user) => {
                            ...
                            if (
                                !student_user.studentDeets.sets.filter(
                                    (qset) => qset.setId == set_id
                                ).length
                            ) {
                                //Only adds set if set not already present
                ...
                const assignedStudents = tutor_user.tutorDeets.sets.find(
                    (qset) => qset._id == set_id
                ).students;
                ...
                tutor_user.tutorDeets.sets
                    .find((qset) => qset._id == set_id)
