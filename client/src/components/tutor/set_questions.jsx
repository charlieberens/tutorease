...
class SetQuestions extends Component {
    ...
    render() {
        ...
        return (
            <Switch>
                <Route
                    path={`/app/tutor/sets/${this.props.id}/create-question`}
                >
                    <CreateQuestion
                        set_id={this.props.id}
                        loadMethod={this.props.loadSet}
                    />
                </Route>
                <Route path={`/app/tutor/sets/${this.props.id}`}>
                    <div>
                        {!this.props.questions?.length ? (
                            <div className="no-questions-cont">
                                <p>
                                    This set doesn't have any questions.{" "}
                                    <Link
                                        className="primary-a"
                                        to={`/app/tutor/sets/${this.props.id}/create-question`}
                                    >
                                        Make one
                                    </Link>
                                    ?
                            ...
                            <div className="question-list">
                                {this.props.questions.map((question, index) => (
                                    <div className="question-list-item">
                                        <span className="question-list-item-number">
                                            {index + 1}
                                        </span>
                                        {this.props.editable && (
                                            <span
                                                className="question-list-item-dots"
                                                onClick={() =>
                                                    this.controlQuestionPopup(
                                                        index,
                                                        true
                                                    )
                                                }
                                        ...
                                        <div className="question-list-item-answer-cont">
                                            {question.answers.map(
                                                (answer, i) => (
                                                    <div
                                                        className={`question-list-item-answer ${
                                                            i === 0
                                                                ? "question-list-item-answer-right"
                                                                : "question-list-item-answer-wrong"
                                                        }`}
                                                    >
                                                        <span className="question-list-item-answer-text">
                                                            <Latex>
                                                                {answer}
                                                            </Latex>
                                                        </span>
                                ...
                                {this.props.editable && (
                                    <Link
                                        className="question-list-create"
                                        to={`/app/tutor/sets/${this.props.id}/create-question`}
                                    >
                                        <IoAddCircle className="question-list-create-plus" />
                    ...
                    {this.state.deleteQuestionPopupOpen && (
                        <Popup popupMethod={this.controlDeleteQuestionPopup}>
                            <DeleteQuestion
                                popupMethod={this.controlDeleteQuestionPopup}
                                loadQuestions={this.props.loadSet}
                                question={this.state.deleteQuestion}
                                set_id={this.props.id}
                            />
                        </Popup>
                    )}
