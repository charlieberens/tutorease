...
class AssignSet extends Component {
    ...
    assign = () => {
        ...
        axios
            .post(
                `/api/students/assign/${students.join(",")}/${
                    this.props.set_id
                }`
            )
            .then((res) => {
    ...
    render() {
        return (
            <div className="assign-student-list">
                <h2>
                    Assign <em>{this.props.set_title}</em> to:
                </h2>
                ...
                {this.state.students.map((student, index) => (
                    <div
                        className={`student-list-item student-list-student assign-student-list-item ${
                            student.selected && "selected"
                        }`}
                        onClick={() => this.selectStudent(index)}
                    >
                        <img
                ...
                <button
                    className="assign-button button-a"
                    onClick={this.assign}
                >
                    Assign
                </button>
