				...
				for(let i=0; i<tutor_user.tutorDeets.sets.length; i++){				
					if(tutor_set_obj[tutor_id].includes(tutor_user.tutorDeets.sets[i]._id.toString())){
						...
						embiggened_sets.push({
							...
							numCorrect: tutor_set.questions.length === student_set.questions.filter(question => question.responses.length).length ? student_set.questions.filter(question => question.responses.length === 1).length : undefined,
